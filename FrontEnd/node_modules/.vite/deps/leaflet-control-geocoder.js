    ...
    getJSON(this.options.serviceUrl + "/findAddressCandidates", params, (data) => {
      ...
      if (data.candidates && data.candidates.length) {
        for (let i = 0; i <= data.candidates.length - 1; i++) {
          ...
          const latLng3 = L.latLng(loc.location.y, loc.location.x);
          const latLngBounds2 = L.latLngBounds(L.latLng(loc.extent.ymax, loc.extent.xmax), L.latLng(loc.extent.ymin, loc.extent.xmin));
          results[i] = {
            name: loc.address,
            bbox: latLngBounds2,
            center: latLng3
    ...
    jsonp(this.options.apiKey, params, (data) => {
      ...
      if (data.resourceSets.length > 0) {
        for (let i = data.resourceSets[0].resources.length - 1; i >= 0; i--) {
          const resource = data.resourceSets[0].resources[i], bbox = resource.bbox;
          results[i] = {
            name: resource.name,
    ...
    getJSON(this.options.serviceUrl, params, (data) => {
      ...
      if (data.results && data.results.length) {
        for (let i = 0; i <= data.results.length - 1; i++) {
          ...
          const latLng3 = L.latLng(loc.geometry.location);
          const latLngBounds2 = L.latLngBounds(L.latLng(loc.geometry.viewport.northeast), L.latLng(loc.geometry.viewport.southwest));
          results[i] = {
    ...
    getJSON(url, params, (data) => {
      ...
      if (data.items && data.items.length) {
        for (let i = 0; i <= data.items.length - 1; i++) {
          ...
          const latLng3 = L.latLng(item.position.lat, item.position.lng);
          let bbox;
          if (item.mapView) {
...
var Mapzen = Pelias;
var mapzen = pelias;
var Openrouteservice = class extends Pelias {
  constructor(options) {
    super(L.Util.extend({
...
var Photon = class {
  ...
  reverse(latLng3, scale, cb, context) {
    const params = reverseParams(this.options, {
      lat: latLng3.lat,
      lon: latLng3.lng
    });
...
var What3Words = class {
  ...
  geocode(query, cb, context) {
    getJSON(this.options.serviceUrl + "forward", geocodingParams(this.options, {
      ...
      const results = [];
      if (data.geometry) {
        const latLng3 = L.latLng(data.geometry["lat"], data.geometry["lng"]);
        const latLngBounds2 = L.latLngBounds(latLng3, latLng3);
        results[0] = {
          name: data.words,
          bbox: latLngBounds2,
          center: latLng3
...
var GeocoderControl = class extends EventedControl {
  /**
  ...
  _geocode(suggest) {
    ...
    const cb = (results) => {
      if (requestCount === this._requestCount) {
        ...
        this.fire(suggest ? "finishsuggest" : "finishgeocode", event2);
  ...
  _createAlt(result, index) {
    const li = L.DomUtil.create("li", ""), a = L.DomUtil.create("a", "", li), icon = this.options.showResultIcons && result.icon ? L.DomUtil.create("img", "", a) : null, text = result.html ? void 0 : document.createTextNode(result.name), mouseDownHandler = (e) => {
...
var Control_Geocoder_modern_default = GeocoderControl;
export {
  GeocoderControl as Geocoder,
  Control_Geocoder_modern_default as default,
  geocoder,
  geocoders
};
